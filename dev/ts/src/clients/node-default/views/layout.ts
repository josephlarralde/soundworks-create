...
class Layout {
  ...
  lastComponentOutputs: Map<string|Component, string>

  constructor(client: Client) {
    this.client = client;
    this.components = new Set();
    this.lastComponentOutputs = new Map(); // <comp, lastStrValue>
  }
  ...
  render() {
    ...
    this.components.forEach(comp => {
      const last = this.lastComponentOutputs.get(comp);
      const value = isComponent(comp) ? comp.render() : comp;

      if (last !== value) {
        console.log(chalk.green(`[${this.client.role} ${this.client.id}]`), value);
        this.lastComponentOutputs.set(comp, value);
      }
    });
  }
}
